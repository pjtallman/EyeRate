...
from matika.i18n import get_text
...
router = APIRouter()
# Templates are in the plugin's template dir
...
async def list_securities(request: Request, accept_language: str = Header(None), user: User = Depends(check_page_permission), db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "admin_securities.html", {
        "t": t, "title": t.get("item_securities"), "user": user, "securities": db.query(FinancialSecurity).all(),
...
@router.post("/securities/update/{sec_id}", tags=[PageType.MAINTENANCE])
...
@router.post("/securities/delete/{sec_id}", tags=[PageType.MAINTENANCE])
async def delete_security(sec_id: int, db: Session = Depends(get_db)):
    sec = db.query(FinancialSecurity).filter(FinancialSecurity.id == sec_id).first()
    if sec: db.delete(sec); db.commit()
    return RedirectResponse(url="/admin/securities", status_code=303)

@router.get("/securities/search", tags=[PageType.MAINTENANCE])
...
@router.get("/securities/lookup", tags=[PageType.MAINTENANCE])
...
@router.post("/securities/bulk_create", tags=[PageType.MAINTENANCE])
...
@router.post("/securities/bulk_delete", tags=[PageType.MAINTENANCE])
